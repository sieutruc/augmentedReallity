...
public class ImageTargets extends Activity
{
    ...
    public native void passNumPages(int numPages);
    public native void passPage(int i, WebPage page);
    ...
    private void loadTextures()
    {
    	...
    	try{
	   	  ...
	   	  FileInputStream fstream = new FileInputStream("/sdcard/chooser.txt");	      	
	   	  DataInputStream in = new DataInputStream(fstream);
	   	  BufferedReader br = new BufferedReader(new InputStreamReader(in));
	   	  strLine = br.readLine();
	      typeID = Integer.valueOf(strLine);
	      in.close();
	    }
	    ...
	    String []types = {"thanhnien", "tuoitre", "SGK"};
	   
    	String filename = "/sdcard/" + types[typeID] + "/" + types[typeID] + ".xml";    
    	AndroidSaxFeedParser parser = new AndroidSaxFeedParser(filename);      
    	
    	List<WebPage> list = parser.parse();
        passNumPages(list.size());
        for(int i = 0; i < list.size(); i++)
        {
        	WebPage page = list.get(i);
	    	for(int j = 0; j < page.numObjects(); j++)
	    	{
	    		MediaObject object = page.get(j);    		
	        	for(int k = 0; k < object.getNumUri(); k++)
	        		object.setUri(k, "/sdcard/" + types[typeID] + "/" + object.getUri(k));
	        	object.initOtherData();
	    	}
        }
        
        mTextures.add(Texture.loadTextureFromApk("/sdcard/Tuoitre/ic_audio.jpg", getAssets()));
        mTextures.add(Texture.loadTextureFromApk("/sdcard/Tuoitre/ic_md2.png", getAssets()));
        int texId = 2;
        ...
        for(int i = 0; i < list.size(); i++)
        {
        	...
        	for(int j = 0; j < page.numObjects(); j++)
        	{
        		...
        		if(object.getType() == 0 || object.getType() == 1)
        		{
	        		for(int k = 0; k < object.getNumUri(); k++)
	        		{
	        			mTextures.add(Texture.loadTextureFromApk(object.getUri(k), getAssets()));
	        			object.setTexId(texId);
	        			//System.out.println("fuck" + object.getTexId() + object.getUri(k));
	        		}
	        		texId += object.getListUri().size();
        		}
        		else 
        			if (object.getType() == 4)
        			{
        				mTextures.add(Texture.loadTextureFromApk(object.getUri(1), getAssets()));
        				object.setTexId(texId);
        				texId++;
        			}
        	}
